#include <iostream>
#include <string>
#include <cmath>
using namespace std;

void addone(int *mas){
  mas[0]=mas[0]+1;
  for(int i =0; i<24;i++){
    if(mas[i]==2){
      mas[i]=0;                      ///////// функція +1
      mas[i+1]=mas[i+1]+1;
    }
    else {i=25;}
  }
}

int main() {
  string v = "";
  int vidp[222];
  for(int i=0;i<222; i++){
  vidp[i]=v[i]-48;  
  }
 
  int a =222;
  int b=71;
  int mas[25]={0};
 // addone(mas);
 // addone(mas);
// for(int i =0;i<222;i++){
 // cout<<"i"<<i<<"="<<vidp[i]<<endl;
  // cout<<vidp[i];
  //}
  
  for(int i=0;i < (int) pow(2,25);i++){
    int r =0;
    int tempmas[25]={0};
    for(int j=0;j<i;j++){
      addone(tempmas);                       //генеруємо всі можливі вектори довжини 25
    } 
    int tempmas2[222]={0};
    for (int i=0;i<25;i++){
      tempmas2[i]=tempmas[i];
    }
    
    for(int k=0;k<222-25;k++){
      if(tempmas2[k]==tempmas2[k+3]){
      tempmas2[k+25]=0;                        // розширюємо за L_1 вектор до довжини 222
        }
      else {tempmas2[k+25]=1;}
    }
    for (int i=0;i<222;i++)В{
      if(vidp[i]!=tempmas2[i]){
        r++;                                      // порівнюємо з відповидним вектором
      }
    }
    if(r<=71){
      for(int i=0;i<222;i++){
        cout<<tempmas[i]<<"  ,r="<<r<<endl;         // виводимо вектор і r
      }
    }
  }
}